...
public class ThumbnailatorDmo {
    public static void main(String[] args) {
        ...
        try {
            ...
            double scale = 0.1;
            Thumbnails.of("D:/test/158383775782.jpg").
            //            size(300, -1).
            // scalingMode(ScalingMode.BICUBIC).
                scale(scale). // 图片缩放80%, 不能和size()一起使用
            // outputQuality(1). // 图片质量压缩80%
            toFile("D:/test/158383775782scale" + scale + ".jpg");

            Image srcFile = ImageIO.read(new File("D:/test/158383775782scale" + scale + ".jpg"));
            System.out.println(scale + "    " + 834 * scale + "   " + 1226 * scale);
            System.out.println(scale + "    " + srcFile.getWidth(null) + "   " + srcFile.getHeight(null));
    ...
    private static void generateRotationWatermark() {
        try {
            Thumbnails.of("data/2016010208.jpg").size(160, 160). // 缩放大小
                rotate(90). // 顺时针旋转90度
    ...
    private static void generateOutputstream() {
        try (OutputStream outputStream = new FileOutputStream("data/2016010208_outputstream.png")) { // 自动关闭流
            Thumbnails.of("data/2016010208.jpg").size(500, 500).outputFormat("png"). // 转换格式
